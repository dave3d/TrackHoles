...
class RegistrationPointDataAquisition(object):
    """
    This class provides a GUI for localizing corresponding points in two
    images, and for evaluating registration results using a linked cursor
    approach, user clicks in one image and the corresponding point is added
    to the other image.
    """

    def __init__(self, fixed_image, moving_image, fixed_window_level=None,
                 moving_window_level=None, figure_size=(10, 8),
                 known_transformation=None):
        self.fixed_image = fixed_image
        self.fixed_npa, self.fixed_min_intensity, self.fixed_max_intensity\
            = self.get_window_level_numpy_array(self.fixed_image,
                                                fixed_window_level)
        self.moving_image = moving_image
        self.moving_npa, self.moving_min_intensity, self.moving_max_intensity\
            = self.get_window_level_numpy_array(self.moving_image,
                                                moving_window_level)
        self.fixed_point_indexes = []
        self.moving_point_indexes = []
        # Keep a history of user point localizations,
        # enabling undo of last localization.
        self.click_history = []

        # If the transformation is valid (not None) then corresponding points
        ...
        # (__call__ method is the invoked callback).
        self.fig.canvas.mpl_connect('button_press_event', self)

        # Display the data and the controls, first time we display the images
        # is outside the "update_display" method as that method relies on the
        # previous zoom factor which doesn't exist yet.
        self.fixed_axes.imshow(self.fixed_npa[self.fixed_slider.value, :, :]
                               if self.fixed_slider else self.fixed_npa,
                               cmap=plt.cm.Greys_r,
        ...
        self.moving_axes.imshow(
            ...
            if self.moving_slider else self.moving_npa,
            cmap=plt.cm.Greys_r,
    ...
    def create_ui(self):
        # Create the active UI components. Height and width are specified
        # in 'em' units. This is a html size specification, size relative
        # to current font size.
        self.viewing_checkbox = widgets.RadioButtons(
            description='Interaction mode:', options=['edit', 'view'],
            value='edit')
        ...
        if self.fixed_npa.ndim == 3:
            self.fixed_slider = widgets.IntSlider(
                description='fixed image z slice:', min=0,
                max=self.fixed_npa.shape[0] - 1, step=1,
                value=int((self.fixed_npa.shape[0] - 1) / 2),
                width='20em')
            self.fixed_slider.observe(self.on_slice_slider_value_change,
                                      names='value')

            self.moving_slider = widgets.IntSlider(
                description='moving image z slice:', min=0,
                max=self.moving_npa.shape[0] - 1, step=1,
                value=int((self.moving_npa.shape[0] - 1) / 2), width='19em')
            self.moving_slider.observe(self.on_slice_slider_value_change,
                                       names='value')

            bx0 = widgets.Box(padding=7,
                              children=[self.fixed_slider, self.moving_slider])

        # Layout of UI components. This is pure ugliness because we are not
        # using a UI toolkit. Layout is done using the box widget and padding
        # so that the visible UI components are spaced nicely.
        ...
        return widgets.HBox(
            children=[widgets.HBox(children=[bx1, bx2, bx3]), bx0])\
            if self.fixed_npa.ndim == 3 else widgets.HBox(
            children=[widgets.HBox(children=[bx1, bx2, bx3])])

    def get_window_level_numpy_array(self, image, window_level):
        """
        Get the numpy array representation of the image and the min and max of
        the intensities used for display.
        """
        npa = sitk.GetArrayViewFromImage(image)
        if not window_level:
            return npa, npa.min(), npa.max()
        else:
            return npa, window_level[1] - window_level[0] / 2.0,\
    ...
    def update_display(self):
        """
        Display the two images based on the slider values, if relevant, and the
        points which are on the displayed slices.
        """
        # We want to keep the zoom factor which was set prior to display, so we
        # log it before clearing the axes.
        ...
        self.fixed_axes.imshow(self.fixed_npa[self.fixed_slider.value, :, :]
                               ...
                               vmin=self.fixed_min_intensity,
                               vmax=self.fixed_max_intensity)
        # Positioning the text is a bit tricky, we position relative to the
        # data coordinate system, but we want to specify the shift in pixels
        # as we are dealing with display. We therefore (a) get the data point
        # in the display coordinate system in pixel units (b) modify the point
        # using pixel offset and transform back to the data coordinate system
        # for display.
        text_x_offset = -10
        text_y_offset = -10
        for i, pnt in enumerate(self.fixed_point_indexes):
            if (self.fixed_slider
                and int(pnt[2] + 0.5) == self.fixed_slider.value)\
                ...
                text_in_data_coords =\
                    self.fixed_axes.transData.inverted().transform(
                        (text_in_data_coords[0] + text_x_offset,
                         text_in_data_coords[1] + text_y_offset))
                self.fixed_axes.text(text_in_data_coords[0],
                                     text_in_data_coords[1], str(i),
                                     color=self.text_and_marker_color)
        self.fixed_axes.set_title('fixed image - localized {0} points'
                                  .format(len(self.fixed_point_indexes)))
        ...
        for i, pnt in enumerate(self.moving_point_indexes):
            if (self.moving_slider and int(pnt[2] + 0.5)
                    == self.moving_slider.value) or not self.moving_slider:
                self.moving_axes.scatter(pnt[0], pnt[1], s=90, marker='+',
                                         color=self.text_and_marker_color)
                text_in_data_coords = self.moving_axes.transData.transform(
                    [pnt[0], pnt[1]])
                text_in_data_coords\
                    = self.moving_axes.transData.inverted().transform(
                          (text_in_data_coords[0] + text_x_offset,
                           text_in_data_coords[1] + text_y_offset))
                self.moving_axes.text(text_in_data_coords[0],
                                      text_in_data_coords[1], str(i),
                                      color=self.text_and_marker_color)
        self.moving_axes.set_title(
            'moving image - localized {0} points'
            .format(len(self.moving_point_indexes)))
        self.moving_axes.set_axis_off()

        # Set the zoom factor back to what it was before we cleared the axes,
        # and rendered our data.
    ...
    def clear_last(self, button):
        """
        Remove last point or point-pair addition (depends on whether the
        interface is used for localizing point pairs or evaluation of
        registration).
        """
    ...
    def get_points(self):
        ...
        if (len(self.fixed_point_indexes) != len(self.moving_point_indexes)):
            raise Exception('Number of localized points in fixed'
                            + ' and moving images does not match.')
        fixed_point_list = [
    ...
    def __call__(self, event):
        ...
        """
        # We add points only in 'edit' mode. If the spatial transformation
        # between the two images is known, self.known_transformation was set,
        # then every button_press_event will generate a point in each of the
        # images. Finally, we enforce that all points have a corresponding
        # point in the other image by not allowing the user to add multiple
        # points in the same image, they have to add points by switching
        # between the two images.
        if self.viewing_checkbox.value == 'edit':
            if event.inaxes == self.fixed_axes:
                if len(self.fixed_point_indexes)\
                        - len(self.moving_point_indexes) <= 0:
                    self.fixed_point_indexes.append(
                        (event.xdata, event.ydata, self.fixed_slider.value)
                        if self.fixed_slider else (event.xdata, event.ydata))
                    self.click_history.append(self.fixed_point_indexes)
                    if self.known_transformation:
                        moving_point_physical\
                            = self.known_transformation.TransformPoint(
                                self.fixed_image
                                .TransformContinuousIndexToPhysicalPoint(
                                    self.fixed_point_indexes[-1]))
                        moving_point_indexes = self.moving_image.\
                ...
                if len(self.moving_point_indexes) -\
                    ...
                    if self.known_transformation:
                        ...
                        if self.fixed_slider:
                            ...
                            if self.fixed_slider.max >= z_index and\
                                    self.fixed_slider.min <= z_index:
...
class PointDataAquisition(object):
    ...
    def __init__(self, image, window_level=None, figure_size=(10, 8)):
        self.image = image
        self.npa, self.min_intensity, self.max_intensity =\
        ...
        # Connect the mouse button press to the canvas (__call__ method is the
        # invoked callback).
        self.fig.canvas.mpl_connect('button_press_event', self)

        # Display the data and the controls, first time we display the image is
        # outside the "update_display" method # as that method relies on the
        # previous zoom factor which doesn't exist yet.
        self.axes.imshow(self.npa[self.slice_slider.value, :, :]
                         if self.slice_slider else self.npa,
                         cmap=plt.cm.Greys_r,
    ...
    def create_ui(self):
        # Create the active UI components. Height and width are specified in
        # 'em' units. This is a html size specification, size relative to
        # current font size.
        self.viewing_checkbox = widgets.RadioButtons(
            description='Interaction mode:', options=['edit', 'view'],
            value='edit')
        ...
        return widgets.HBox(
            children=[widgets.HBox(children=[bx1, bx2, bx3]), bx0])\
            if self.slice_slider else widgets.HBox(
            children=[widgets.HBox(children=[bx1, bx2, bx3])])
    ...
    def update_display(self):
        ...
        for i, pnt in enumerate(self.point_indexes):
            if (self.slice_slider and int(pnt[2] + 0.5)
                    == self.slice_slider.value) or not self.slice_slider:
                self.axes.scatter(pnt[0], pnt[1], s=90, marker='+',
                                  color='yellow')
                # Get point in pixels.
                text_in_data_coords = self.axes.transData.transform([pnt[0],
                                                                     pnt[1]])
                # Offset in pixels and get in data coordinates.
                text_in_data_coords = self.axes.transData.inverted().transform(
                    (text_in_data_coords[0] + text_x_offset,
                     text_in_data_coords[1] + text_y_offset))
                self.axes.text(text_in_data_coords[0], text_in_data_coords[1],
                               str(i), color='yellow')
        self.axes.set_title(
            'localized {0} points'.format(len(self.point_indexes)))
    ...
    def validate_points(self, point_index_data):
        for p in point_index_data:
            if self.npa.ndim != len(p):
                raise ValueError(
                    'Given point (' + ', '.join(map(str, p)) +
                    ') dimension does not match image dimension.')
            outside_2d_bounds = p[0] >= self.npa.shape[2] or p[0] < 0\
                or p[1] >= self.npa.shape[1] or p[1] < 0
            outside_bounds = outside_2d_bounds or (
                False if self.npa.ndim == 2
                else p[2] >= self.npa.shape[0] or p[2] < 0)
            if outside_bounds:
                raise ValueError('Given point (' + ', '.join(map(str, p)) +
    ...
    def get_points(self):
        return [self.image.TransformContinuousIndexToPhysicalPoint(pnt)
    ...
    def get_point_indexes(self):
        ...
        return [tuple(map(lambda x: int(round(x)), pnt))
                for pnt in self.point_indexes]
        ...
        if self.viewing_checkbox.value == 'edit':
            if event.inaxes == self.axes:
                self.point_indexes.append(
                    ...
                    if self.slice_slider else (event.xdata, event.ydata))
                self.update_display()


def multi_image_display2D(image_list, title_list=None, window_level_list=None,
                          figure_size=(10, 8), horizontal=True):
    if title_list:
    ...
    # Create a figure.
    col_num, row_num = (len(image_list), 1) if horizontal\
        else (1, len(image_list))
    ...
    if not window_level_list:
        ...
        min_intensity_list = list(map(lambda x: x[1] - x[0] / 2.0,
                                      window_level_list))
        max_intensity_list = list(map(lambda x: x[1] + x[0] / 2.0,
                                      window_level_list))

    # Draw the image(s)
    for ax, npa, title, min_intensity, max_intensity\
        in zip(axes, npa_list, title_list, min_intensity_list,
               max_intensity_list):
        ax.imshow(npa,
                  cmap=plt.cm.Greys_r,
                  vmin=min_intensity,
...
class MultiImageDisplay(object):
    ...
    be specified by the user as input to the constructor or set via the
    displayed slider. For color images the intensity control slider will be
    disabled. This allows us to display both color and grayscale images in the
    same figure with a consistent look to the controls. The range of the
    intensity slider is set to be from top/bottom 2% of intensities
    (accomodating for outliers). Images are displayed either in horizontal or
    vertical layout, depending on the users choice.
    '''

    def __init__(self, image_list, axis=0, shared_slider=False,
                 title_list=None, window_level_list=None,
        ...
        if title_list:
            if len(image_list) != len(title_list):
                raise ValueError(
                    'Title list and image list lengths do not match')
        ...
        # Create a figure.
        col_num, row_num = (len(image_list), 1) if horizontal else\
            (1, len(image_list))
        self.fig, self.axes = plt.subplots(row_num, col_num,
                                           figsize=figure_size)
        if len(image_list) == 1:
            self.axes = [self.axes]

        # Display the data and the controls, first time we display the image
        # is outside the "update_display" method as that method relies on the
        # previous zoom factor which doesn't exist yet.
        for ax, npa, slider, wl_slider in zip(self.axes, self.npa_list,
                                              self.slider_list, self.wl_list):
            self.slc[self.axis] = slice(slider.value, slider.value + 1)
            # Need to use squeeze to collapse degenerate dimension
            # (e.g. RGB image size 124 124 1 3)
    ...
    def create_ui(self, shared_slider, wl_range, wl_init):
        ...
        if shared_slider:
            # Validate that all the images have the same size along the axis
            # which we scroll through
            sz = self.npa_list[0].shape[self.axis]
            for npa in self.npa_list:
                if npa.shape[self.axis] != sz:
                    raise ValueError(
                        'Not all images have the same size along'
                        + 'the specified axis, cannot share slider.')
            ...
            for npa in self.npa_list:
                slider = widgets.IntSlider(
                    description='image slice:', min=0,
                    max=npa.shape[self.axis] - 1, step=1,
                    value=int((npa.shape[self.axis] - 1) / 2), width='20em')
                slider.observe(self.on_slice_slider_value_change,
                               names='value')
        ...
        # color and grayscale images in the same UI while retaining a
        # reasonable layout for the sliders.
        for r_values, i_values, npa in zip(wl_range, wl_init, self.npa_list):
            wl_range_slider = widgets.IntRangeSlider(
                description='intensity:', min=r_values[0], max=r_values[1],
                step=1, value=[i_values[0], i_values[1]], width='20em',
                disabled=len(npa.shape) == 4)
            wl_range_slider.observe(self.on_wl_slider_value_change,
                                    names='value')
    ...
    def get_window_level_numpy_array(self, image_list, window_level_list,
        ...
        # to the original images. If they are deleted outside the view would
        # become invalid, so we use a copy, which guarantees that the gui is
        # consistent.
        ...
        for i, npa in enumerate(npa_list):
            if len(npa.shape) == 4:  # color image
                ...
                # We don't necessarily take the minimum/maximum values, just in
                # case there are outliers user can specify how much to take off
                # from top and bottom.
                ...
                if not window_level_list:  # No list was given.
                    ...
                    if wl:
                        wl_init.append((wl[1] - wl[0] / 2.0,
                                        wl[1] + wl[0] / 2.0))
                    else:
                        # We have a list, but for this image the entry was
                        # left empty: []
                        wl_init.append(wl_range[-1])
    ...
    def update_display(self):
        ...
        # Draw the image(s)
        for ax, npa, title, slider, wl_slider in zip(self.axes, self.npa_list,
                                                     self.title_list,
                                                     self.slider_list,
                                                     self.wl_list):
            # We want to keep the zoom factor which was set prior to display,
            # so we log it before clearing the axes.
            xlim = ax.get_xlim()
            ylim = ax.get_ylim()
            ...
            # Set the zoom factor back to what it was before we cleared the
            # axes, and rendered our data.
            ax.set_xlim(xlim)
            ax.set_ylim(ylim)
...
class ROIDataAquisition(object):
    '''
    This class provides a GUI for selecting box shaped Regions Of Interest
    (ROIs). Each ROI is represented as a tuple: ((min_x,max_x),(min_y,max_y),
    and possibly (min_z,max_z)) if dealing with a 3D image. When using the
    zoom/pan tool from the toolbar ROI selection is disabled. Once you click
    again on the zoom/pan button zooming/panning will be disabled and ROI
    selection is enabled. Note that when you are marking the ROI on a slice
    that is outside the Z-range selected by the range slider, once you are
    done selecting the ROI, you will see no change on the current slice. This
    is the correct behavior, though initially you may be surprised by it.
    '''
    ...
    def __init__(self, image, window_level=None, figure_size=(10, 8)):
        ...
        self.roi_display_properties = dict(facecolor='red', edgecolor='black',
                                           alpha=0.2, fill=True)
        ...
        self.roi_selector = RectangleSelector(
            self.axes, lambda eclick, erelease: None,
        ...
        # Display the data and the controls, first time we display the image is
        # outside the "update_display" method # as that method relies on the
        # existance of a previous image which is removed from the figure.
        self.axes.imshow(self.npa[self.slice_slider.value, :, :]
                         if self.slice_slider else self.npa,
                         cmap=plt.cm.Greys_r,
                         vmin=self.min_intensity,
    ...
    def create_ui(self):
        ...
        if self.npa.ndim == 3:
            self.roi_range_slider = widgets.IntRangeSlider(
                description='ROI z range:', min=0, max=self.npa.shape[0] - 1,
                step=1, value=[0, self.npa.shape[0] - 1], width='20em')
            bx4 = widgets.Box(padding=15, children=[self.roi_range_slider])
            self.slice_slider = widgets.IntSlider(
                description='image z slice:', min=0, max=self.npa.shape[0] - 1,
                step=1, value=int((self.npa.shape[0] - 1) / 2), width='20em')
            self.slice_slider.observe(self.on_slice_slider_value_change,
                                      names='value')
            bx0 = widgets.Box(padding=7, children=[self.slice_slider])
        ...
        return widgets.HBox(
            ...
            if self.npa.ndim == 3 else widgets.HBox(
            children=[widgets.HBox(children=[bx1, bx2, bx3])])
    ...
    def get_window_level_numpy_array(self, image, window_level):
        npa = sitk.GetArrayViewFromImage(image)
        # We don't take the minimum/maximum values, just in case there are
        # outliers (top/bottom 2%)
        if not window_level:
    ...
    def update_display(self):
        ...
        # Iterate over all of the ROIs and only display/undisplay those that
        # are relevant.
        if self.slice_slider:
            for roi_data in self.rois:
                if self.slice_slider.value >= roi_data[3][0]\
    ...
    def add_roi_data(self, roi_data):
        ...
        for roi in roi_data:
            self.rois.append((patches.Rectangle((roi[0][0], roi[1][0]),
                              ...
                              roi[0], roi[1], roi[2] if self.npa.ndim == 3
                              else None))
    ...
    def set_rois(self, roi_data):
        ...
        Input is an iterable containing tuples where each tuple contains
        two or three tuples (min_x,max_x),(min_y,max_y), (min_z,max_z)
        depending on the image dimensionality. The ROI is the box defined
        by these integer values and includes both min/max values.
        '''
        self.clear_all_data()
        ...
        for roi in roi_data:
            for i, bounds in enumerate(roi, 1):
                if bounds[0] > bounds[1]:
                    raise ValueError(
                        'First element in each tuple is expected'
                        + ' to be smaller than second element, error in ROI ('
                        + ', '.join(map(str, roi)) + ').')
                # Note that SimpleITK uses x-y-z specification vs numpy's z-y-x
                if not (bounds[0] >= 0 and bounds[1] <
                        self.npa.shape[self.npa.ndim - i]):
                    raise ValueError(
                        'Given ROI (' + ', '.join(map(str, roi))
                        + ') is outside the image bounds.')
    ...
    def add_roi(self, button):
        if self.roi_selector.visible:
            ...
            roi_extent = [int(round(coord)) for coord in
                          self.roi_selector.extents]
            ...
            self.rois.append((patches.Rectangle((roi_extent[0], roi_extent[2]),
                              ...
                              self.roi_range_slider.value if
                              self.roi_range_slider else None))
            self.axes.add_patch(self.rois[-1][0])
            self.update_display()
    ...
    def get_rois(self):
        '''
        Return a list of tuples representing the ROIs. Each tuple contains
        either two or three tuples (min_x,max_x), (min_y,max_y), (min_z,max_z)
        depending on image dimensionality. The ROI is the box defined by these
        integer values and includes both min/max values.
        '''
        return [(roi_data[1], roi_data[2], roi_data[3])
                if self.npa.ndim == 3 else (roi_data[1], roi_data[2])
                for roi_data in self.rois]

    def __call__(self, event):
        # This is dangerous as we are accessing a "private" variable to find
        # the state of the figure's toolbar ('ZOOM',PAN' or None). When Zoom or
        # pan are active we will ignore the button press, once the user
        # deactivates the zoom/pan we can allow them to select the ROI.
        # Discussion on stack overflow with matplotlib developer (circa 2013),
        # no change to date:
...
class PairedPointDataManipulation(object):
    '''
    This class provides a GUI for paired point creation, to illustrate the use
    of the SimpleITK LandmarkBasedTransformInitializer. The GUI displays a
    region of size [0,data_scale]X[0,data_scale] in which the user can select
    points. The user can then translate and rotate the point configuration. The
    points are either fiducials (used in registration) or targets (not used in
    registration). The transformation estimated by the component can be either
    rigid or affine. The UI allows the user to add noise, bias,... to the
    moving fiducials and perform registration.
    '''

    def __init__(self, transform=sitk.Euler2DTransform(), data_scale=100.0,
                 figure_size=(8, 6)):
        self.figure_size = figure_size

        # Properties of the glyphs used to display the fixed/moving
        # fiducials/targets and rotation centroid.
        self.FIXED_FIDUCIAL_CONFIG = {'marker': 'x', 'markersize': 6,
                                      'color': "#ff8888"}
        self.FIXED_TARGET_CONFIG = {'marker': 's', 'markerfacecolor': 'none',
                                    'markersize': 6, 'color': "#88ff88"}
        self.MOVING_FIDUCIAL_CONFIG = {'marker': '+', 'markersize': 8,
                                       'color': "#ffdddd"}
        self.MOVING_TARGET_CONFIG = {'marker': 'o', 'markerfacecolor': 'none',
                                     'markersize': 8, 'color': "#ddffdd"}
        self.CENTROID_CONFIG = {'marker': '.', 'markersize': 4,
                                'color': "#58d8ff"}

        # The fixed fiducials/targets do not have noise added to them.
        self.fixed_fiducials = []
        self.fixed_targets = []
        # The moving fiducials will have noise, bias added to their
        # coordinates. Noise model is zero mean Gaussian, isotropic and
        # homogenous.
        ...
        # Fiducials can only be added before any noise/bias is added to the
        # existing fiducials
        self.can_add_fiducials = True

        # The transformation type that will be used by the
        # LandmarkBasedTransformInitializer # this component supports,
        # Rigid2DTransform, AffineTransform, BSplineTransform
        ...
        # Create a figure.
        self.fig, self.axes = plt.subplots(1, 1, True, True,
    ...
    def create_ui(self):
        ...
        # current font size.
        self.viewing_checkbox = widgets.RadioButtons(
            description='Mode:', options=['edit', 'translate', 'rotate'],
            value='edit')
        ...
        return widgets.HBox(children=[bx0, widgets.VBox(
            children=[widgets.HBox([bx1, bx2, bx3, bx4]),
    ...
    def update_display(self):
        ...
        # Draw the fixed and moving fiducials and targets using the glyph
        # specifications defined in the class constructor.
        self.moving_fiducials_glyphs = []
        self.moving_targets_glyphs = []
        for fixed_fiducial, moving_fiducial in zip(self.fixed_fiducials,
                                                   self.moving_fiducials):
            self.axes.plot(fixed_fiducial[0], fixed_fiducial[1],
                           **(self.FIXED_FIDUCIAL_CONFIG))
            self.moving_fiducials_glyphs += self.axes.plot(
                moving_fiducial[0], moving_fiducial[1],
                **(self.MOVING_FIDUCIAL_CONFIG))
        for fixed_target, moving_target in zip(self.fixed_targets,
                                               self.moving_targets):
            self.axes.plot(fixed_target[0], fixed_target[1],
                           **(self.FIXED_TARGET_CONFIG))
            self.moving_targets_glyphs += self.axes.plot(
                moving_target[0], moving_target[1],
                **(self.MOVING_TARGET_CONFIG))
        if self.centroid:
            self.axes.plot(self.centroid[0], self.centroid[1],
        ...
        # Set the data range back to what it was before we cleared the axes,
        # and rendered our data.
    ...
    def outlier(self, button):
        if self.moving_fiducials:
            ...
            new_x = max(min(self.moving_fiducials[index][0] + 0.1 * self.scale,
                            self.scale), 0)
            new_y = max(min(self.moving_fiducials[index][1] + 0.1 * self.scale,
                            self.scale), 0)
            ...
            if self.can_add_fiducials:
                if event.button == 1:  # left click
                    if event.inaxes == self.axes:
                        ...
                        self.moving_fiducials.append([event.xdata,
                                                      event.ydata])
    ...
    def on_motion(self, event):
        if event.button == 1:  # left click
            if self.viewing_checkbox.value == 'translate':
                ...
                for glyph in self.moving_fiducials_glyphs +\
                        self.moving_targets_glyphs:
                ...
                for glyph in self.moving_fiducials_glyphs\
                    ...
                    px = glyph.get_xdata()
                    py = glyph.get_ydata()
                    glyph.set_data(ox + np.cos(angle) * (px - ox)
                                   - np.sin(angle) * (py - oy),
                                   oy + np.sin(angle) * (px - ox)
                                   + np.cos(angle) * (py - oy))
                self.previous = v2
                self.fig.canvas.draw_idle()
                self.fig.canvas.flush_events()
    ...
    def on_release(self, event):
        if event.button == 1:  # left click
            if self.viewing_checkbox.value == 'translate' or\
                    self.viewing_checkbox.value == 'rotate':
                # Update the actual data using the glyphs (modified during
                # translation/rotation)
                for glyph, fiducial in zip(self.moving_fiducials_glyphs,
                                           self.moving_fiducials):
                    fiducial[0] = float(glyph.get_xdata())
                    fiducial[1] = float(glyph.get_ydata())
                for glyph, target in zip(self.moving_targets_glyphs,
                                         self.moving_targets):
    ...
    def get_FLE(self):
        return [np.sqrt(fle_vec[0] ** 2 + fle_vec[1] ** 2) for fle_vec
                in self.FLE]

    def get_all_data(self):
        return (self.fixed_fiducials, self.fixed_targets,
                self.moving_fiducials, self.moving_targets, self.get_FLE())
    ...
    def set_points(self, points, are_fiducials=True):
        ...
        if min(all_coords) < 0 or max(all_coords) > self.scale:
            raise ValueError(
                'One of the points is outside the image bounds, ' +
                '[0,{0}]X[0,{0}].'.format(self.scale))
        # Copy the data in and coerce points to lists. The coercion to list
        # allows us to accept tuples, as internally we need the points to be
        # mutable.
        fill_lists = [self.fixed_fiducials, self.moving_fiducials]\
            if are_fiducials else [self.fixed_targets, self.moving_targets]
        for p in points:
...
def display_errors(fixed_fiducials, fixed_targets, FLE_errors, FRE_errors,
                   TRE_errors, min_err=None, max_err=None,
    ...
    print("Mean FLE %.6f\t STD FLE %.6f\t Min FLE %.6f\t Max FLE %.6f"
          % FLE_errors[0:4])
    print("Mean FRE %.6f\t STD FRE %.6f\t Min FRE %.6f\t Max FRE %.6f"
          % FRE_errors[0:4])
    print("Mean TRE %.6f\t STD TRE %.6f\t Min TRE %.6f\t Max TRE %.6f"
          % TRE_errors[0:4])
